...
pure function evaluate_polynomial(x, order, coefficients) result(y)
    ! Input variables
    real, intent(in) :: x
    integer, intent(in) :: order
    real, dimension(order+1), intent(in) :: coefficients
...
function parallel_quadrature(order, coefficients, bounds, num_pts_in, num_cpu_in) result(quad)
    ...
    y = evaluate_polynomial(xi, order, coefficients)
    quad = quad + 0.5 * y
    y = evaluate_polynomial(xf, order, coefficients)
    quad = quad + 0.5 * y
...
!$OMP DO REDUCTION(+:quad)
    do i = 2, num_pts-1
        x = xi + dx*(i-1)
        y = evaluate_polynomial(x, order, coefficients)
        quad = quad + y
    end do
!$OMP END DO
!$OMP END PARALLEL
